...
class FFChatInfo {
  ...
  final List<UsuariosRecord>? groupMembers;

  UsuariosRecord get currentUser => groupMembers!
      .where((user) => user.reference == currentUserReference)
      .first;
  Map<String, UsuariosRecord> get otherUsers => Map.fromEntries(
        groupMembers!
            .where((user) => user.reference != currentUserReference)
            .map((user) => MapEntry(user.reference.id, user)),
      );
  List<UsuariosRecord> get otherUsersList => otherUsers.values.toList();
...
class FFChatManager {
  ...
  Stream<FFChatInfo> getChatInfo({
    UsuariosRecord? otherUserRecord,
    DocumentReference? chatReference,
    ...
    return chatStream.asyncMap((chat) async {
      ...
      final groupMembers = await Future.wait(
        userRefs.map((ref) => UsuariosRecord.getDocument(ref).first),
      );
